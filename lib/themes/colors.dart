import 'package:flutter/material.dart';

const white255 = Color(0xFFFFFFFF);
const white250 = Color(0xFFFAFAFA);
const white245 = Color(0xFFF5F5F5);
const white240 = Color(0xFFF0F0F0);
const white235 = Color(0xFFEBEBEB);
const white230 = Color(0xFFE6E6E6);

const black0 = Color(0xFF000000);
const black10 = Color(0xFF0A0A0A);
const black20 = Color(0xFF141414);
const black30 = Color(0xFF1E1E1E);
const black40 = Color(0xFF282828);
const black50 = Color(0xFF323232);

const grey80 = Color(0xFF505050);
const grey120 = Color(0xFF787878);
const grey160 = Color(0xFFA0A0A0);
const grey200 = Color(0xFFC8C8C8);

const errorRed = Color(0xFFBF4040);
const errorRedLight = Color(0xFFF6C1C1);

const successGreen = Color(0xFF40BF40);
const successGreenLight = Color(0xFFC1F6C1);

const warningYellow = Color(0xFFBFBF40);
const warningYellowLight = Color(0xFFF6F6C1);

const brown = Color(0xFFA5785E);
const brownDark = Color(0xFF866255);
const brownLight = Color(0xFFDBCBBB);
const brownWhite = Color(0xFFEAE5D9);

const brownWhite245 = Color(0xFFFAF5E9);
const brownWhite235 = Color(0xFFF0EBDF);
const brownWhite225 = Color(0xFFE6E1D5);

const brownBlack20 = Color(0xFF191408);
const brownBlack40 = Color(0xFF2D281C);
const brownBlack60 = Color(0xFF413C30);

const brownGrey80 = Color(0xFF555044);
const brownGrey120 = Color(0xFF7D786C);
const brownGrey160 = Color(0xFFA5A094);
const brownGrey200 = Color(0xFFCDC8BC);

const orange = Color(0xFFDEAF5F);
const orangeDark = Color(0xFFCA7923);
const orangeLight = Color(0xFFF0DCB7);
const orangeWhite = Color(0xFFF9F1E2);

const green = Color(0xFF7AA433);
const greenDark = Color(0xFF264F0A);
const greenLight = Color(0xFFBCCF9A);
const greenWhite = Color(0xFFEFF4E6);

class CustomColors {
  static Color bgColor1 = white230;
  static Color bgColor2 = white235;

  static Color fgColor1 = brownBlack20;
  static Color fgColor2 = brownBlack40;
  static Color fgColor3 = brownBlack60;
  static Color fgColor4 = brownGrey80;

  static Color borderColor1 = brownGrey160;
}
